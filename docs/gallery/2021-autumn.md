...
活动规则
0. Java版与基岩版的比试将被分为两个赛区，请勿在错误的赛区提交你的截图！ 
1. 我们推荐使用社区内已进驻创作者（包括比赛期间新进驻的创作者）的任何已发布光影或材质，但付费的内容需要自行购买。 
2. 不得进行外部/后期修改。你可以调整着色器的所有设置甚至魔改一些内部设置（需要自行取得作者同意，无需通知管理）。但是，截图在拍摄后不允许修改。 
3. 每个用户只能提交一张，不允许额外提交。如果你违反了这个规则超过一次，你可能会被取消资格。 
4. 允许使用自定义地图和Mods。(请在你的简短描述中说明你所使用的东西） 
5. 在活动分区的频道不进行文字聊天。只用于提交你的图片与简单描述。 
6. 禁止剽窃他人作品，一经发现，取消比赛资格。
