...
- 调查表明，持错误观点的玩家可能受以下几种混淆说法及因素影响：
  - **认为可以使用非光追手段实现光线追踪效果：**
    - `光线追踪技术` 可作为实现 **全局光照** 或一些其它效果（如**高光、PBR、丁达尔效应、体积光，阴影**）的一种手段，这些效果使用 `非光线追踪一类技术` 确实也能实现。
    - 但按照这种说法走下去，非光线追踪技术也理应实现一些后者 `独有` 的技术，如小孔成像、光的色散等。
    - 因此只能够说 `非光线追踪技术同样能够实现全局光照等一些效果` ，而非**错误地认为**实现光线追踪技术能够做到的部分效果=伪光追。
