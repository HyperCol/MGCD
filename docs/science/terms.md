...
- **资源包**：Resource Pack，即通俗所说的材质包，详见：[资源包相关](resourcepacks.md#关于资源包纹理包和材质包) 。
...
- **着色器**：Shader，光影中包含一个或数个着色器，在渲染中，每个**像素或顶点**会运算都会运算**所有**的片段着色器 (Fragment Shader) 或顶点着色器 (Vertex Shader) 代码。
  - 除此之外，还有计算着色器 (Compute Shader)、几何着色器 (Geometry Shader)，它们主要用以抽象计算和顶点生成。
...
- **SSS**：此简写包含以下两种概念
  ...
  - **焦散**：经过**一次或数次**反射或折射后，由经漫反射表面反射最终到达观察者视角的光线，可以称之为焦散。--Tahnass
