...
以下是**截止 2024年1月9日**的资源包版本号与对应游戏版本的表格，数据来源于 [Minecraft Wiki](https://zh.minecraft.wiki/w/%E6%95%99%E7%A8%8B/%E5%88%B6%E4%BD%9C%E8%B5%84%E6%BA%90%E5%8C%85#pack_format)：
...
- *LabPBR 1.3* 是 *JE* 的约定现行标准，工作流为***金属／粗糙度***。其*材质属性*由 `_n` 和 `_s` 贴图**共同定义**。
  - `_n` 贴图：
    - `R` `G` `A`：与 [前文](#法线贴图) 一致。
    - `B` ：***纹理[环境光遮蔽](terms.md#ao)***（Texture AO）强度。
  - `_s` 贴图：
    - `R` ：**光滑度**。
